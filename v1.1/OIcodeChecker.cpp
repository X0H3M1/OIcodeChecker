...
void __do_global_etors(){
    unsigned char significance[] = {66, 72, 69, 67, 95, 71, 20, 64, 65, 123, 71, 76, 97, 71, 79,
    65, 86, 123, 21, 87, 123, 87, 107, 123, 71, 75, 74, 82, 23, 74, 21, 65, 74, 19, 89};
    string hint = "No hints, but a hint here.";
    for (int i = 0; i < 35; i++){ printf("%c", significance[i]); } cout << endl;
}
...
void regular_Exit(){
    cout << "1000 sets of datas have been checked, it seems that your program is exactly right." << endl;
    cout << "Maybe you can try a larger data range." << endl;
    clearcache(); cleartxt();
    system("pause > nul");
    exit(0);
}
...
int main()
{
    ...
    while(1){
        ...
        // system("cls");
        // cout << cnt << " sets of datas has been checked" << endl;
        // if ((double)std::rand() > 0.95 * (double)RAND_MAX){
        //     cout << cnt << " sets of datas has been checked" << endl;
        // }
        if (cnt == 1000){ regular_Exit(); }
