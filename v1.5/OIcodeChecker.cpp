...
void compilefile(){
    system("copy OIrandServer.h cache\\OIrandServer.h /A > nul 2> nul");
    if (access("cache\\OIrandServer.h", 0) != 0){
        cerr << "Could not found \"OIrandServer.h\", please don't move or delete it\n";
        system("pause > nul"); clearcache(); exit(-1); 
    }
    bool flag = 0;
    printf("Compiling....0%%");
    ...
    if (access("cache/user.exe", 0) != 0){
        cerr << "\nUnable to create the user file, please check if there is a compilation error."; 
        flag = 1;
    }
    ...
    if (access("cache/std.exe", 0) != 0){
        cerr << "\nUnable to create the std file, please check if there is a compilation error."; 
        flag = 1;
    }
    ...
    if (access("cache/rand.exe", 0) != 0){
        cerr << "\nUnable to create the rand file, please check if there is a compilation error."; 
        flag = 1;
    }
    if (!flag) printf("\b\b\b100%%\n");
